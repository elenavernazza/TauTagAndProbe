...
void PureRate()
{
          ...
          if (L1_objects_VBF)
            {
              ...
              PtJet1_PtJet2->Fill(myGoodOnlineJet1.Pt(),myGoodOnlineJet2.Pt());
              ...
              //     cout << "\nEvent number " << i << endl;
              //     cout << "\nOnline Muons = ";
              //     for (UInt_t a = 0; a < in_l1tMuPt->size(); ++a) cout << "[" << in_l1tMuPt->at(a) << ", " << in_l1tMuEta->at(a) << ", " << in_l1tMuPhi->at(a) << "] ";
              //     cout << "\nOnline Jets = ";
              //     for (UInt_t a = 0; a < in_l1tPtJet->size(); ++a) cout << "[" << in_l1tPtJet->at(a) << ", " << in_l1tEtaJet->at(a) << ", " << in_l1tPhiJet->at(a) << "] ";
              //     cout << "\nGoodMuon = [" << myGoodOnlineMuon.Pt() << ", " << myGoodOnlineMuon.Eta() << ", " << myGoodOnlineMuon.Phi() << "]\n" << endl;
              //     cout << IsolatedMuon << endl;
              //   }
                      ...
                      if (n_cut == 0)
                        {
                          if (myGoodOnlineMuon.Pt() < muon_cut_max)
                            {
                              ptjet1_ptjet2_jetmass_ptmu.at(n_cut)->Fill(myGoodOnlineJet1.Pt(), myGoodOnlineJet2.Pt(), myGoodOnlineDiJet.M());
  ...
  for (Int_t I_bin_y = 1 ; I_bin_y <= bins[1] ; ++I_bin_y)
    {
      for (Int_t I_bin_x = I_bin_y ; I_bin_x <= bins[0] ; ++I_bin_x)
        {
  ...
  rate_2D->SetStats(0);
  rate_2D->Draw("COLZ");
  rate_2D->SetMinimum(0);
  rate_2D->SetMaximum(5);
  ...
  // Compute 2D_rate with a fixed muonPt value and jetPt2 value 
  TH2F* rate_2D_ptj1_mjj = new TH2F("rate_2D_ptj1_mjj","rate_2D_ptj1_mjj", bins[0], xmin[0], xmax[0], bins[2], xmin[2], xmax[2]);
      ...
      for (Int_t I_bin_z = 1 ; I_bin_z <= bins[2] ; ++I_bin_z)
        {
          ...
          for (UInt_t i_mu = 6; i_mu < ptjet1_ptjet2_jetmass_ptmu.size(); ++i_mu)
            {
              rate_value += ptjet1_ptjet2_jetmass_ptmu.at(i_mu)->Integral(I_bin_x,bins[0]+1,7,bins[1]+1,I_bin_z,bins[2]+1)/Denominator*scale;
            }
  ...
  rate_2D_ptj1_mjj->GetZaxis()->SetTitleOffset(0.6);
  ...
  for (Int_t m = 0; m < bins[3]; m++)
    {
      ...
      ptjet1_ptjet2_jetmass_ptmu.at(m)->SetMaximum(100);
      gPad->Update();
