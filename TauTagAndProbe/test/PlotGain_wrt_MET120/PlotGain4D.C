...
bool CheckMET (TTree* inTree, UInt_t i_ev)
  {
    ...
    Float_t myGoodOfflineMET;
    Float_t myGoodOnlineMET;
    ...
    if (offline_objects_VBF && L1_objects_VBF)
      {
        ...
        bool check_mu_offline   = myGoodOfflineMuon.Pt() > 3; // [FIXME] Change threshold
        ...
        bool check_MET_Offline = check_met_offline && check_mu_offline && check_jet1_offline && check_jet2_offline;
...
void CheckVBF (TTree* inTree, UInt_t  i_ev, vector<array<Float_t, 4>> set_of_on_cuts, vector<array<Float_t, 4>> set_of_off_cuts, bool pass_MET, vector<UInt_t>* acceptance_VBF, vector<UInt_t>* acceptance_MET_VBF)
  {
...
void PlotGain()
  {
    ...
    TString FileName_rate = "/grid_mnt/data__data.polcms/cms/vernazza/CMSSW_10_2_1/src/TauTagAndProbe/TauTagAndProbe/test/FindGoodRates/Rates_4D.root";
    ...
    TString Path_ntuples = "/grid_mnt/data__data.polcms/cms/vernazza/CMSSW_10_2_1/src/TauTagAndProbe/TauTagAndProbe/test/MC_MiniAOD_EWKino_DemocraticSlepton_mChargino_17_11_22/";
        ...
        for (UInt_t i_ev = 0 ; i_ev < 10000 ; ++i_ev)
          {
            ...
            bool pass_MET = CheckMET(inTree, i_ev);
            if (pass_MET)
              {
                ++ acceptance_MET;
              }
            // The VBF trigger is computed for all the possible good combinations of selections
            CheckVBF(inTree, i_ev, set_of_on_cuts, set_of_off_cuts, pass_MET, &acceptance_VBF, &acceptance_MET_VBF);
    ...
    for (Int_t muon_pt = xmin[3]; muon_pt < xmax[3]; ++muon_pt)
      {
        ...
        PtJet1_PtJet2_Mjj.at(m)->SetMaximum(0.71);
        ...
        PtJet1_PtJet2_Mjj.at(m)->SaveAs(Form("Gain_3D_MET120_PtJet1_PtJet2_Mjj_PtMuon%i.root", muon_pt));
        c.SaveAs(Form("Gain_3D_MET120_PtJet1_PtJet2_Mjj_PtMuon%d.png", muon_pt));
        c.SaveAs(Form("Gain_3D_MET120_PtJet1_PtJet2_Mjj_PtMuon%d.pdf", muon_pt));
      }
...
// Plot gain 2D for fixed ptj2 and ptmu
  ...
  Tex33.DrawLatexNDC(0.25,0.96,"Gain p_{T}^{jet2} > 30 GeV && p_{T}^{#mu} > 6 GeV wrt MET120");
  Tex33.Draw("same");

  c_2D->SaveAs("Gain_2D_MET120_ptj1_X_ptj2_30_mjj_Y_ptmu_6.png");
  c_2D->SaveAs("Gain_2D_MET120_ptj1_X_ptj2_30_mjj_Y_ptmu_6.pdf");
  c_2D->Close();
