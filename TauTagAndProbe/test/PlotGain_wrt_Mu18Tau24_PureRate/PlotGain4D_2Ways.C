...
void PlotGain2D (Float_t fixed_cut_y, Float_t fixed_cut_w, Int_t bins[4], Double_t xmin[4], Double_t xmax[4], vector<array<Int_t, 4>> set_of_on_cuts, vector<array<Float_t, 4>> set_of_on_bins, vector<UInt_t> acceptance_VBF, vector<UInt_t> acceptance_MuTau_VBF, UInt_t acceptance_MuTau, TString Output)
  {

    // Plot gain 2D for fixed ptj2 and ptmu
    TH2F* gain_2D = new TH2F("gain_2D","gain_2D", bins[0], xmin[0], xmax[0], bins[2], xmin[2], xmax[2]);
    cout << bins[0] << xmin[0] << xmax[0] << bins[2] <<  xmin[2] <<  xmax[2] << endl;
        ...
        if (set_of_on_cuts.at(i_c)[1] == fixed_cut_y && set_of_on_cuts.at(i_c)[3] == fixed_cut_w)
          {
            ...
            cout << gain << endl;
            cout << gain_2D->GetBinContent(set_of_on_bins.at(i_c)[0], set_of_on_bins.at(i_c)[2]) << endl;
          }
      }
    ...
    c_2D->SaveAs(Output+"/Gain_2D_Mu18Tau24_ptj1_X_ptj2_"+to_string(fixed_cut_y)+"_mjj_Y_ptmu_"+to_string(fixed_cut_w)+".png");
    c_2D->SaveAs(Output+"/Gain_2D_Mu18Tau24_ptj1_X_ptj2_"+to_string(fixed_cut_y)+"_mjj_Y_ptmu_"+to_string(fixed_cut_w)+".pdf");
    c_2D->Close();
