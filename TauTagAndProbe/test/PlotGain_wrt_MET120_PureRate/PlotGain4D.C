...
// Check if a given event passes selections for the acceptance: offline + online + matching for all objects involved in L1MET120 trigger
bool CheckMET (TTree* inTree, UInt_t i_ev, TString JetIDType, TString Method, bool JetSel30)
  {
    ...
    bool check_MET = false;

    // Check if there are at least 2 jets and 1 mu both in the online and offline objects
    bool offline_objects_VBF = in_PxJet->size() > 1 && in_MuPt->size() > 0;

    if (offline_objects_VBF)
      {
        Float_t myOfflineMET = in_pfMetNoMuEt;
        Float_t myOnlineMET = in_l1t_MET_mt->at(0);

        // cout << "Offline MET = " << in_pfMetNoMuEt << " GeV" << endl;
        // cout << "Online MET = " << in_l1t_MET_mt->at(0) << " GeV" << endl;

        // [FIXME] Maybe add selection on the MET type!
        ...
        for (UInt_t i_mu = 0 ; i_mu < in_MuPt->size() ; ++i_mu)
          {
            ...
            if (CheckGoodMuon(in_MuIso->at(i_mu), in_MuID->at(i_mu)))
              {
        ...
        bool check_mu_offline   = myGoodOfflineMuon.Pt() > 4;
        ...
        bool check_met_offline  = myOfflineMET > 200.;

        bool check_MET_Offline = check_met_offline && check_mu_offline && check_jet1_offline && check_jet2_offline && check_mjj_offline;
        bool check_MET_Online = myOnlineMET > 120.;

        check_MET = check_MET_Offline && check_MET_Online;
...
void CheckVBF (TTree* inTree, UInt_t  i_ev, vector<array<Float_t, 4>> set_of_on_cuts, vector<array<Float_t, 4>> set_of_off_cuts, bool pass_MET, vector<UInt_t>* acceptance_VBF, vector<UInt_t>* acceptance_MET_VBF, TString JetIDType, TString Method, bool JetSel30)
  {
            ...
            if (check_mu_matching && check_jet1_matching && check_jet2_matching)
              {
                ...
                bool check_mjj_online  = myGoodOnlineDiJet.M() > set_of_on_cuts.at(i_cut)[2];
                ...
                bool check_met_offline  = in_pfMetNoMuEt > 10; // [FIXME] maybe remove

                check_VBF_online = check_jet1_online && check_jet2_online && check_mjj_online && check_mu_online;
                check_VBF_offline = check_jet1_offline && check_jet2_offline && check_mjj_offline && check_mu_offline && check_met_offline;
...
// Loops over the SUSY events ntuples and computes the VBF, MET and common acceptance
...
void PlotGain(TString EventSample)
  {
    ...
    UInt_t acceptance_MET = 0;
    vector<UInt_t> acceptance_VBF;
    vector<UInt_t> acceptance_MET_VBF;

    cout << "Initialize Acceptance arrays" << endl;
    for (UInt_t i_cut = 0; i_cut < set_of_on_cuts.size(); ++i_cut)
      {
        acceptance_VBF.push_back(0.);
        acceptance_MET_VBF.push_back(0.);
      }

    TString JetIDType = "None";
   ...
   TString Path_ntuples = "/grid_mnt/data__data.polcms/cms/vernazza/Ntuples/"+EventSample;
    TString Output = "/grid_mnt/data__data.polcms/cms/vernazza/CMSSW_10_2_1/src/TauTagAndProbe/TauTagAndProbe/test/PlotGain_wrt_MET120_PureRate/"+EventSample+"_GainPlots_"+JetIDType+"_"+JetSel30;
    system("mkdir -p "+Output);

    // loop over all events in the ntuples to check which one passes the VBF or MET selections
    for (int nt = 0 ; nt < 5 ; ++nt) // [FIXME] Actually we have much more ntuples
      {
        ...
        for (UInt_t i_ev = 0 ; i_ev < 10000 ; ++i_ev)
          {
            // The MET trigger is computed separately, since it has fixed thresholds values
            bool pass_MET = CheckMET(inTree, i_ev, JetIDType, Method, checkJetSel30);
            if (pass_MET)
              {
                ++ acceptance_MET;
              }
            // The VBF trigger is computed for all the possible good combinations of selections
            CheckVBF(inTree, i_ev, set_of_on_cuts, set_of_off_cuts, pass_MET, &acceptance_VBF, &acceptance_MET_VBF,  JetIDType, Method, checkJetSel30);
        ...
        for (Int_t muon_pt = xmin[3]; muon_pt < xmax[3]; ++muon_pt)
          {
            if (set_of_on_cuts.at(i_c)[3] == muon_pt)
              {
                int m = muon_pt - xmin[3];
                float gain = Float_t(acceptance_VBF.at(i_c) - acceptance_MET_VBF.at(i_c))/Float_t(acceptance_MET);
    ...
    TString rootname = Output+"/Gain_3D_MET120_PtJet1_PtJet2_Mjj_PtMuon.root";
    ...
    for (Int_t muon_pt = xmin[3]; muon_pt < xmax[3]; ++muon_pt)
      {
        ...
        Tex3.DrawLatexNDC(0.5,0.93,Form("Gain (p_{T}^{#mu} > %d [GeV]) wrt MET120", muon_pt));
        Tex3.Draw("same");
    ...
    // gain is computed as the number of the events passing only the VBF trigger (and not the MET trigger),
    //  with respect to the number of events passing only the MET trigger
    ...
    for (UInt_t i_c = 0; i_c < set_of_on_cuts.size(); ++i_c)
      {
        Float_t Acceptance_gain = Float_t(acceptance_VBF.at(i_c) - acceptance_MET_VBF.at(i_c))/Float_t(acceptance_MET);
        if (Acceptance_gain > 0.11 && set_of_on_cuts.at(i_c)[2] < 500)
          {
            cout << "Acceptance cut [" << set_of_on_cuts.at(i_c)[0] << "," << set_of_on_cuts.at(i_c)[1] << "," << set_of_on_cuts.at(i_c)[2] << "," << set_of_on_cuts.at(i_c)[3] << "] VBF = " << acceptance_VBF.at(i_c) << " VBF+MET = " << acceptance_MET_VBF.at(i_c) << endl;
            cout << "Gain = " << Float_t(acceptance_VBF.at(i_c) - acceptance_MET_VBF.at(i_c))/Float_t(acceptance_MET) << endl;
          }
...
// Plot gain 2D for fixed ptj2 and ptmu
  ...
  gain_2D->GetZaxis()->SetTitleOffset(1.1);
  gain_2D->SetTitle("");
  gain_2D->SetStats(0);
  gain_2D->Draw("COLZ");
  gain_2D->Draw("text same");
  gain_2D->SetMinimum(0.);
  gain_2D->SetMaximum(max(1.,1.3*gain_2D->GetMaximum()));
  ...
  TString printout = "Gain p_{T}^{jet2} > " + to_string(int(fixed_cut_y)) + " GeV && p_{T}^{#mu} > " + to_string(int(fixed_cut_w)) + " GeV wrt MET120";

  Tex33.DrawLatexNDC(0.25,0.96,printout);
  Tex33.Draw("same");

  c_2D->SaveAs(Output+"/Gain_2D_MET120_ptj1_X_ptj2_"+to_string(fixed_cut_y)+"_mjj_Y_ptmu_"+to_string(fixed_cut_w)+".png");
  c_2D->SaveAs(Output+"/Gain_2D_MET120_ptj1_X_ptj2_"+to_string(fixed_cut_y)+"_mjj_Y_ptmu_"+to_string(fixed_cut_w)+".pdf");
  c_2D->Close();
